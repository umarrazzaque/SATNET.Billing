...
namespace SATNET.WebApp.BackgroundTasks
{
    public class CustomQuartzHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            var MRCJobMetaData = new JobMetadata(Guid.NewGuid(), typeof(MRCJob), "MCR Job", "0 0 0 1 * ?"); //(live) At 00:00:00am, on the 1st day, every month 
            //var MRCJobMetaData = new JobMetadata(Guid.NewGuid(), typeof(MRCJob), "MCR Job", "0 */2 * ? * *"); // // Every 2 minutes
            //var MRCJobMetaData = new JobMetadata(Guid.NewGuid(), typeof(MRCJob), "MCR Job", "0 */30 * ? * *"); // Every 30 minutes
            ...
            //var endOfMonthJobMetaData = new JobMetadata(Guid.NewGuid(), typeof(EndOfMonthJob), "End of month job", "0 */2 * ? * *"); // Every 1 minutes
            ...
            var tenthOfMonthJobMetaData = new JobMetadata(Guid.NewGuid(), typeof(TenthOfMonthJob), "Tenth of month Job", "0 0 0 10 * ?"); //(live) At 00:00:00am, on the 10th day, every month
